...
# Format the output
formatted_height=$(echo "$height" | awk '{print $1/10}')  # Convert to meters
formatted_weight=$(echo "$weight" | awk '{print $1}')      # Append kg

# Construct the formatted output message
formatted_output="${name^} is of type $type, weighs ${formatted_weight}kg, and is ${formatted_height}m tall."

# Output the formatted message
echo "$formatted_output"

# Check if the script contains the formatted output
if ! grep -qF "$formatted_output" "$SCRIPT_FILE"; then
    echo "Error: '$SCRIPT_FILE' doesn't contain: [$formatted_output]" >> $ERROR_FILE
    exit 1
fi
